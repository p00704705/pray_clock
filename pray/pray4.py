...
class PrayerReminder(QWidget):
    ...
    def initUI(self):
        ...
        self.play_button.setStyleSheet("color: white; background-color: green;")
        self.play_button.clicked.connect(self.play_prayer_sound)
    ...
    def check_prayer_time(self):
        now = datetime.now()
        if now.strftime('%H:%M') in prayer_times:
            self.play_prayer_sound()
            self.next_prayer_time = self.get_next_prayer_time()

    def play_prayer_sound(self):
        try:
